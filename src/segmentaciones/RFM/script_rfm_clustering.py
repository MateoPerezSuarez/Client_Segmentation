...
# Choose the optimal k based on the elbow method (user-defined or observed from the plot)
optimal_k = 4  # Replace with the optimal value you identify

# Apply K-Means clustering with the optimal number of clusters
kmeans = KMeans(n_clusters=optimal_k, random_state=42)
rfm['Cluster'] = kmeans.fit_predict(rfm_scaled)

# Visualize clusters in a 2D scatter plot (Recency vs. Monetary)
plt.figure(figsize=(8, 6))
plt.scatter(rfm['Recency'], rfm['Monetary'], c=rfm['Cluster'], cmap='viridis', alpha=0.6)
plt.xlabel('Recency (days)')
plt.ylabel('Monetary Value')
plt.title(f'Customer Segments (k={optimal_k}) based on RFM')
